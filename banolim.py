...
def query_detail15(service_key, chem_id):
    try:
        ...
        for item in root.findall('.//item'):
            ...
            # O02: 산업안전보건법에 의한 규제
            if code == 'O02':
                if '관리대상유해물질' in detail:
                    result['관리대상유해물질'] = True
                if '특별관리물질' in detail:
                    result['특별관리물질'] = True
                if any(detail.strip().startswith(prefix) for prefix in ['특수건강진단대상물질']):
                    result['특수건강진단대상물질'] = True

            # O04, O12: 화학물질관리법 / 화학물질 등록평가법
            elif code in ['O04', 'O12']:
                if '인체급성유해성물질' in detail:
                    result['인체급성유해성물질'] = True
                if '인체만성유해성물질' in detail:
...
st.title("📋 화학물질 유해성 정보 수집기 v.250902_1")
...
if uploaded_file and not st.session_state.processed:
    ...
    # ✅ A열~AS열(1~45열)만 제목행 검사
    header_row = header_row_full[:45]
    ...
    for r_idx, row in hazard_df.iterrows():
        ...
        for col_name in HAZARD_ORDER:
            if col_name in col_name_to_idx:
                ...
                if col_name in ['#', 'CAS No.', '결과없음', '개정일','관리대상유해물질', '특별관리물질', '특수건강진단대상물질', '인체급성유해성물질', '인체만성유해성물질']:
